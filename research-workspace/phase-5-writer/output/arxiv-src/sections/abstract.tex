...
Our headline result is a measurement: applying the Miller--Sanjurjo
streak-selection correction to cricket --- to our knowledge for the first time ---
we quantify the apparent-momentum illusion at the death over at $+0.121$, the gap
between the naive and bias-corrected per-ball momentum estimators. That illusion
nearly fully accounts for the apparent ``cold hand'': the naive estimate
($D=-0.114$) is almost exactly the finite-sequence bias ($-0.121$), so the
corrected estimate is $+0.007$ and not significant against a state-stratified null
($p=0.40$). Because death-over stays are short --- the regime where the bias is
largest --- this is precisely the setting where the uncorrected estimator is most
misleading. On the identity question, we find no evidence that our
