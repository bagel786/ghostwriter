...
at higher $k$. We find it is largely a finite-sequence selection artifact: the
Miller--Sanjurjo bias is nearly equal and opposite, the corrected estimate is
near zero, and no cell survives a state-stratified null with multiple-testing
control. Death-over stays are short, which is the regime where the bias is largest,
so it is a setting where omitting the correction is particularly consequential. We
frame the conclusion as an absence of evidence rather than evidence of absence:
after correction and state control we find no statistically significant per-ball
momentum, which is not the same as proving momentum cannot exist at any scale or for
any player. We report the $+0.121$ gap as the study's main quantity: it estimates
how much of the apparent momentum is introduced by the estimator rather than
...
only. Fourth, the H3 permutation count was $B = 2000$ (from a preregistered $5000$)
for runtime; given the large separation between bias and corrected effect this
affects $p$-value resolution, not the conclusions. Fifth, the dot-run pressure
